...
namespace UiLib {
	...
	void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("header")) == 0 ) GetHeader()->SetVisible(_tcscmp(pstrValue, _T("hidden")) != 0);
			...
			rcTextPadding = this->m_pManager->GetDpiValue(rcTextPadding);
			...
			szCheckImg = this->m_pManager->GetDpiValue(szCheckImg);
			...
			szIconImg = this->m_pManager->GetDpiValue(szIconImg);
