...
namespace UiLib
{
	...
	void CControlUI::SetUseEllipse(bool v)
	{
		m_bUseEllipse = v;
		Invalidate();
	}
	...
	void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("pos")) == 0 ) {
			...
			rcPos = this->m_pManager->GetDpiValue(rcPos);
			...
			szMove = this->m_pManager->GetDpiValue(szMove);
			szZoom = this->m_pManager->GetDpiValue(szZoom);
			...
			rcPadding = this->m_pManager->GetDpiValue(rcPadding);
			...
			if(nValue.Find(',') < 0)
				SetBorderSize(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
			else
			{
				RECT rcPadding = { 0 };
				LPTSTR pstr = NULL;
		...
		else if (_tcscmp(pstrName, _T("leftbordersize")) == 0) SetLeftBorderSize(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("topbordersize")) == 0) SetTopBorderSize(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("rightbordersize")) == 0) SetRightBorderSize(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("bottombordersize")) == 0) SetBottomBorderSize(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
			...
			cxyRound = this->m_pManager->GetDpiValue(cxyRound);
			SetBorderRound(cxyRound);
		}
		else if( _tcscmp(pstrName, _T("bkimage")) == 0 ) SetBkImage(pstrValue);
		else if (_tcscmp(pstrName, _T("width")) == 0) SetFixedWidth(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("height")) == 0) SetFixedHeight(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("minwidth")) == 0) SetMinWidth(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("minheight")) == 0) SetMinHeight(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("maxwidth")) == 0) SetMaxWidth(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		else if (_tcscmp(pstrName, _T("maxheight")) == 0) SetMaxHeight(this->m_pManager->GetDpiValue(_ttoi(pstrValue)));
		...
		else if (_tcscmp(pstrName, _T("useellipse")) == 0) SetUseEllipse(_tcscmp(pstrValue, _T("true")) == 0);
	}
	...
	void CControlUI::DoPaint(HDC hDC, const RECT& rcPaint)
	{
		...
		if (m_bUseEllipse) {
			...
			CRenderClip::GenerateEllipseClip(hDC, m_rcPaint, m_rcItem, roundClip);
			PaintBkColor(hDC);
			PaintBkImage(hDC);
			PaintStatusImage(hDC);
			PaintText(hDC);
			PaintBorder(hDC);
		}else if( m_cxyBorderRound.cx > 0 || m_cxyBorderRound.cy > 0 ) {
			CRenderClip roundClip;
	...
	UINT CWindowControlUI::GetControlFlags() const {
		return UIFLAG_HASCHILWINDOW;
	}

	BOOL CWindowControlUI::PreMessageHandler(LPMSG, LRESULT& lr)
	{
		return FALSE;
	}
} // namespace UiLib
