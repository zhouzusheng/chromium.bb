				...
				if( _tcscmp(pstrClass, _T("Image")) == 0 ) {
					...
					for( int i = 0; i < nAttributes; i++ ) {
						...
						if( _tcscmp(pstrName, _T("name")) == 0 ) {
							...
							size = pManager->GetDpiValue(size);
						}
				...
				if( pManager->GetPaintWindow() ) {
					...
					for( int i = 0; i < nAttributes; i++ ) {
						...
						if( _tcscmp(pstrName, _T("size")) == 0 ) {
							...
							cx = pManager->GetDpiValue(cx);
							cy = pManager->GetDpiValue(cy);
							...
							rcSizeBox = pManager->GetDpiValue(rcSizeBox);
							...
							rcCaption = pManager->GetDpiValue(rcCaption);
