...
#define IDB_ICONS50 203
	...
	void CPaintManagerUI::SetResourceZip(LPVOID pVoid, unsigned int len)
	{
		...
		if( m_bCachedResourceZip ) 
			m_hResourceZip = (HANDLE)OpenZip(pVoid, len, NULL);
	}
	...
	int  CPaintManagerUI::GetDpiValue(int v)
	{
		return int(getScreenScaleFactor() * v);
	}
	float  CPaintManagerUI::GetDpiValue(float v)
	{
		return getScreenScaleFactor() * v;
	}
	SIZE  CPaintManagerUI::GetDpiValue(const SIZE& v)
	{
		return SIZE{ GetDpiValue(v.cx), GetDpiValue(v.cy) };
	}
	POINT  CPaintManagerUI::GetDpiValue(const POINT& v)
	{
		return POINT{ GetDpiValue(v.x), GetDpiValue(v.y) };
	}


	RECT  GetGlobalDpiValue(const RECT& v)
	{
		RECT r;
		r.left =  int(getScreenScaleFactor() * v.left);
		r.top = int(getScreenScaleFactor() * v.top);
		r.right = int(getScreenScaleFactor() * v.right);
		r.bottom = int(getScreenScaleFactor() * v.bottom);
		return r;
	}

	int  GetGlobalDpiValue(int v)
	{
		return int(getScreenScaleFactor() * v);
	}

	RECT  CPaintManagerUI::GetDpiValue(const RECT& v)
	{
		return RECT{ GetDpiValue(v.left), GetDpiValue(v.top), GetDpiValue(v.right), GetDpiValue(v.bottom) };
	}
	...
	void CPaintManagerUI::ChangeFocus(CControlUI* pControl)
	{
		if (pControl == m_pFocus) return;
		if (m_pFocus != NULL)
		{
	...
	CWindowControlUI* CPaintManagerUI::FindControl(HWND hwnd) const
	{
		ASSERT(m_pRoot);
		return(CWindowControlUI*) m_pRoot->FindControl(__FindControlFromHWND, &hwnd, UIFIND_VISIBLE | UIFIND_TOP_FIRST);
	}
	...
	CControlUI* CALLBACK CPaintManagerUI::__FindControlFromHWND(CControlUI* pThis, LPVOID pData)
	{
		HWND* hwindow = static_cast<HWND*>(pData);
		if (pThis->GetControlFlags()&UIFLAG_HASCHILWINDOW)
		{
			CWindowControlUI* pUI = static_cast<CWindowControlUI*>(pThis);
			if (pUI->GetWindow() == *hwindow || ::IsChild(pUI->GetWindow(), *hwindow))
			{
				return pUI;
			}
		}
	...
	bool CPaintManagerUI::TranslateMessage(const LPMSG pMsg)
	{
		...
		if (uChildRes != 0)
		{
			...
			for( int i = 0; i < m_aPreMessages.GetSize(); i++ ) 
			{
				...
				HWND hPrevWnd = pMsg->hwnd;
				while(hTempParent)
				{
					if(pMsg->hwnd == pT->GetPaintWindow() || hTempParent == pT->GetPaintWindow())
					{	
						//现在处理一下Focus 消息
						CWindowControlUI* ui = pT->FindControl(hPrevWnd);
						...
						//给 control 机会处理消息
						if (ui && ui->PreMessageHandler(pMsg, lRes)) {
							return true;
						}
					...
					hPrevWnd = hTempParent;
					hTempParent = GetParent(hPrevWnd);
				}
			}
		}
		else
