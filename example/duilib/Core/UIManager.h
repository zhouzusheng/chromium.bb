...
#define UIFLAG_HASCHILWINDOW    0x10000000
...
typedef struct tagTProperty
{
	...
	CDuiString CalCurImageDest(LPCTSTR _SrcValue,int _TotalFrame,int _CurFrame,bool _IsStartNone,bool _IsEndNone){
		...
		rcSrcValue = GetGlobalDpiValue(rcSrcValue);
...
public:
	...
	int  GetDpiValue(int v);
	float  GetDpiValue(float v);
	SIZE  GetDpiValue(const SIZE& v);
	POINT  GetDpiValue(const POINT& v);
	RECT  GetDpiValue(const RECT& v);
	...
	void ChangeFocus(CControlUI* pControl);
	...
	class CWindowControlUI* FindControl(HWND hwnd) const;
...
private:
	...
	static CControlUI* CALLBACK __FindControlFromHWND(CControlUI* pThis, LPVOID pData);
private:
    HWND m_hWndPaint;
	int m_nOpacity;
    HDC m_hDcPaint;
