...
void CRenderClip::GenerateEllipseClip(HDC hDC, RECT rc, RECT rcItem, CRenderClip& clip)
{
	RECT rcClip = { 0 };
	::GetClipBox(hDC, &rcClip);
	clip.hOldRgn = ::CreateRectRgnIndirect(&rcClip);
	...
	do 
	{
		if( type == NULL ) {
			...
			if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
				...
				else hz = OpenZip(sFile.GetData(), NULL);
				if( hz == NULL ) break;
				ZIPENTRY ze; 
				int i; 
        ...
        while( *pStrImage != _T('\0') ) {
            ...
            if( !sValue.IsEmpty() ) {
                if( sItem == _T("file") || sItem == _T("res") ) {
					...
					rcItem.left = rc.left + GetGlobalDpiValue(_tcstol(sValue.GetData(), &pstr, 10));  ASSERT(pstr);
					rcItem.top = rc.top + GetGlobalDpiValue(_tcstol(pstr + 1, &pstr, 10));    ASSERT(pstr);
					rcItem.right = rc.left + GetGlobalDpiValue(_tcstol(pstr + 1, &pstr, 10));  ASSERT(pstr);
					if (rcItem.right > rc.right) rcItem.right = rc.right;
					rcItem.bottom = rc.top + GetGlobalDpiValue(_tcstol(pstr + 1, &pstr, 10)); ASSERT(pstr);
