...
class CONTENT_EXPORT WebContentsViewWin
 ...
 public:
  ...
  // blpwtk2's WebViews can be placed in windows that belong to different
  // threads or processes.  Because of this, we cannot add a filter hook to the
  // root window's wndproc, because we will run into race conditions because
  // the filter hook code is not threadsafe.  As a workaround, we disable
  // hooking onto the root window, and we will depend on the application to
  // notify us when the root window receives WM_WINDOWPOSCHANGED or
  // WM_SETTINGCHANGE.
