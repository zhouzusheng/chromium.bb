...
String StylePropertySet::get4Values(const CSSPropertyLonghand& longhand) const
{
    ...
    const CSSProperty* top = findPropertyWithId(longhand.properties()[0]);
    const CSSProperty* right = findPropertyWithId(longhand.properties()[1]);
    const CSSProperty* bottom = findPropertyWithId(longhand.properties()[2]);
    const CSSProperty* left = findPropertyWithId(longhand.properties()[3]);
...
String StylePropertySet::getCommonValue(const CSSPropertyLonghand& longhand) const
{
    ...
    for (unsigned i = 0; i < longhand.length(); ++i) {
        ...
        bool currentPropertyIsImportant = propertyIsImportant(longhand.properties()[i]);
